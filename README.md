...
为了保持 Cookie 长期有效，建议安装浏览器插件实现自动同步：

1.  在管理面板点击 **🔌 插件连接**。
2.  点击 **📥 下载油猴脚本** (需先安装 Tampermonkey 扩展) 或加载 Chrome 扩展。
3.  在脚本/扩展设置中填入管理面板显示的 **服务器地址** 和 **连接 Token**。
4.  在浏览器登录 Gemini 官网。
5.  插件会自动提取 Cookie 并同步到您的服务器，且后续会自动在后台保活。
